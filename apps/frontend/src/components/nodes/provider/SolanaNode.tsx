...
import blockTemplateService, { BlockFunctionTemplate, BlockFunctionTemplateParameters } from '../../services/blockTemplateService';
import { OutputDefinition } from '@/types/OutputTypes';
import { nodeUtils } from '@/utils/nodeUtils';
import { FlowNode } from '../../../../../backend/src/packages/compiler/src/types';

interface SolanaNodeProps {
  id: string;
}

export default function SolanaNode({ id }: SolanaNodeProps) {
  const { setNodes } = useReactFlow();
  const [selectedFunction, setSelectedFunction] = useState<string>('');
  const [parameters, setParameters] = useState<Record<string, string>>({});
  const blockFunctionTemplates = blockTemplateService.getTemplatesByType('SOLANA');
  ...
  }, [nodes, id, selectedFunction, blockFunctionTemplates]);
  ...
  }, [id, setNodes, blockFunctionTemplates]);
  ...
  const output: OutputDefinition = useMemo(() => {
    ...
    return {
      ...
      label: 'Result',
      type: 'object',
      description: 'Solana operation result'
    };
  ...
  return (
    <TemplateNode
      id={id}
      metadata={nodeTypes['SOLANA'].metadata}
      inputs={inputs}
