...
export const getSignaturesForAddressDisplayString = `export const getSignaturesForAddress = async (params: Record<string, any>) => {
  try {
    const { 
      address,
      before = undefined,
...
export const getSignaturesForAddressExecuteString = `async function getSignaturesForAddress(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
