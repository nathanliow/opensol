...
export const getVoteAccountsDisplayString = `export const getVoteAccounts = async (params: Record<string, any>) => {
  try {
    const { 
      network = 'devnet' 
    } = params;
...
export const getVoteAccountsExecuteString = `async function getVoteAccounts(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
