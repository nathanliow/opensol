...
export const getRecentPerformanceSamplesDisplayString = `export const getRecentPerformanceSamples = async (params: Record<string, any>) => {
  try {
    const { 
      limit = 100,
      network = 'devnet' 
...
export const getRecentPerformanceSamplesExecuteString = `async function getRecentPerformanceSamples(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
