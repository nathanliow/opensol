...
export const getBlocksDisplayString = `export const getBlocks = async (params: Record<string, any>) => {
  try {
    const { 
      blockNumbers, 
      network = 'devnet' 
...
export const getBlocksExecuteString = `async function getBlocks(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
