...
export const getBlockCommitmentDisplayString = `export const getBlockCommitment = async (params: Record<string, any>) => {
  try {
    const { 
      blockNumber, 
      network = 'devnet' 
...
export const getBlockCommitmentExecuteString = `async function getBlockCommitment(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
