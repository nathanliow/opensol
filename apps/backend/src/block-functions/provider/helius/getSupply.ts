...
export const getSupplyDisplayString = `export const getSupply = async (params: Record<string, any>) => {
  try {
    const { 
      network = 'devnet' 
    } = params;
...
export const getSupplyExecuteString = `async function getSupply(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
