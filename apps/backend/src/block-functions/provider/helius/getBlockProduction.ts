...
export const getBlockProductionDisplayString = `export const getBlockProduction = async (params: Record<string, any>) => {
  try {
    const { 
      network = 'devnet' 
    } = params;
...
export const getBlockProductionExecuteString = `async function getBlockProduction(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
