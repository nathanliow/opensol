...
export const getCompressedBalanceByOwnerDisplayString = `export const getCompressedBalanceByOwner = async (params: Record<string, any>) => {
  try {
    const { 
      owner,
      network = 'devnet' 
...
export const getCompressedBalanceByOwnerExecuteString = `async function getCompressedBalanceByOwner(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
