...
export const isBlockhashValidDisplayString = `export const isBlockhashValid = async (params: Record<string, any>) => {
  try {
    const { 
      network = 'devnet' 
    } = params;
...
export const isBlockhashValidExecuteString = `async function isBlockhashValid(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
