...
export const getTraderGainersAndLosersDisplayString = `export const getTraderGainersAndLosers = async (params: Record<string, any>) => {
  try {
    const { 
      address,
      timeframe = '1W',
...
export const getTraderGainersAndLosersExecuteString = `async function getTraderGainersAndLosers(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
