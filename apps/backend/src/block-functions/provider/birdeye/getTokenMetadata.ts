...
export const getTokenMetadataDisplayString = `export const getTokenMetadata = async (params: Record<string, any>) => {
  try {
    const { 
      address,
      network = 'mainnet'
...
export const getTokenMetadataExecuteString = `async function getTokenMetadata(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
