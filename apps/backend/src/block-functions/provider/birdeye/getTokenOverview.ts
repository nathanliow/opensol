...
export const getTokenOverviewDisplayString = `export const getTokenOverview = async (params: Record<string, any>) => {
  try {
    const { 
      address,
      network = 'mainnet',
...
export const getTokenOverviewExecuteString = `async function getTokenOverview(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
