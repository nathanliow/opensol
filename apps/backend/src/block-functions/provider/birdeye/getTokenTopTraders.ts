...
export const getTokenTopTradersDisplayString = `export const getTokenTopTraders = async (params: Record<string, any>) => {
  try {
    const { 
      address,
      timeframe = '24h',
...
export const getTokenTopTradersExecuteString = `async function getTokenTopTraders(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
