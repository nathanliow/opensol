...
export const getPairOverviewDisplayString = `export const getPairOverview = async (params: Record<string, any>) => {
  try {
    const { 
      address,
      network = 'mainnet'
...
export const getPairOverviewExecuteString = `async function getPairOverview(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
