...
export const getTokenTradesDisplayString = `export const getTokenTrades = async (params: Record<string, any>) => {
  try {
    const { 
      address,
      sort_by = 'block_unix_time',
...
export const getTokenTradesExecuteString = `async function getTokenTrades(params) {
  try {
    const filteredParams = Object.fromEntries(
      Object.entries(params).filter(([key, value]) => value !== "" && value !== null)
    );
